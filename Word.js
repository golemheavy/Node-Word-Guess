...
module.exports = function Word(wordObj) {
	...
	this.letterArray = [];
	for (x in this.wordStrArr) {this.letterArray.push(new letter(this.wordStrArr[x]));};
	this.displayWord = function() {	return this.letterArray.map((x) => {return x.toString()}).join(" "); };
